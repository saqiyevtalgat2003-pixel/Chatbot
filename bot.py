    ...
    messages = [
        {
            ...
            "content": (
                ...
                "сөйлесетін чат-ботсың. Жауабың табиғи, қысқа (1-2 сөйлем) болсын, "
                "тым формальды болма."
            ),
        },
        {
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ботты mention еткен немесе оның хабарламасына reply жасаған болса —
    # Groq арқылы мағыналы AI жауап береміз.
    if is_mentioning_bot(update, context):
        await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
        ai_reply = await generate_ai_reply(chat_id, text)
        if ai_reply:
            await update.message.reply_text(ai_reply)
        else:
            await update.message.reply_text("Қазір ойлана алмай тұрмын 🤔")
        return

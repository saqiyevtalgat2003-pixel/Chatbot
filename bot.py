...
async def generate_ai_reply(chat_id: int, user_text: str) -> str | None:
    if not GROQ_API_KEY:
        return None
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "Сен қазақ тілінде сөйлейтін, Telegram тобындағы достарымен әзілдесіп "
                "сөйлесетін чат-ботсың. Біреу 'сен кімсің' деп сұраса, өзіңді топтың "
                "чат-боты екеніңді қысқаша, әзілмен таныстыр. Жауабың табиғи, қысқа "
                "(1-2 сөйлем) болсын, тым формальды болма."
            ),
        },
        {
            "role": "user",
...
def settings_keyboard(chat_id: int, settings: dict) -> InlineKeyboardMarkup:
    ...
    ai_label = "🤖 AI үнемі жауап беру: ✅" if settings.get("ai_always_reply", True) else "🤖 AI үнемі жауап беру: ❌"
    target = settings.get("force_sub_target") or "орнатылмаған"
    rows = [
        [InlineKeyboardButton(ai_label, callback_data=f"set:ai:{chat_id}")],
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "ai":
        new_val = not settings.get("ai_always_reply", True)
        await upsert_settings(chat_id, ai_always_reply=new_val)
        settings["ai_always_reply"] = new_val
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # AI баптауы қосулы болса — топтағы кез келген хабарламаға Groq арқылы
    # жауап береміз (mention/reply шарты керек емес).
    ai_always = settings.get("ai_always_reply", True)
    if GROQ_API_KEY and (ai_always or is_mentioning_bot(update, context)):
        await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
